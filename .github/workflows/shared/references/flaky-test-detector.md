# Flaky-test detector procedures

Read the section needed for the current phase, not the whole procedure upfront. The
[workflow entry point](../../flaky-test-detector.agent.md) owns authorization, scan commands, and caps.
...
6. Validate the affected projects (Step 7), then open **exactly one**
   combined PR (Step 8).

## Step 1 — Run the detector

Run the new-flake command from the workflow entry point via the `bash` tool.
...
Use the backlog command from the entry point. Its `-MinSources 1` is essential: the script filters
`flakyTests` by this threshold, so a higher threshold would hide single-source failures and could
not establish the zero-failure condition for un-quarantining.
...
- `flakyTests` = quarantined tests with **any observed failure** in 344 under this scan's
  `MinSources = 1` threshold. Leave these **quarantined**; no action this run.
...
- If the **new-flake** scan's `scanComplete` (Step 1) is `false`, that scan was truncated and is biased.
  **Do not file issues, do not quarantine, and do not open a PR from it.** Record the limitation
  and stop the new-flake flow; the independent backlog scan may still justify an action.
...
Work through the strongest candidates in `flakyTests` within the action/time budgets (these already
meet the `MinSources >= 3` issue threshold). Decide
...
Otherwise assess the positive evidence for nondeterminism: spread across genuinely independent
sources/days and a failure mode consistent with a flake, rather than a shared broken baseline.
Different hashes alone can reflect paths or arguments in the same deterministic error. If flake
versus regression remains ambiguous, leave it for human triage rather than quarantining by default.
...
- **If a related issue is OPEN:** plan at most one `add_comment` to that issue with genuinely **new**
  evidence (sources/build URLs not already reported). If it is included in today's PR, combine that
  evidence with the action and temporary PR reference at Step 8, rather than commenting twice.
  Render rolling build IDs as links as described below. Do not open a duplicate.
...
- **Otherwise (no related issue):** create exactly one issue via `create_issue`. **Note:**
  ...
  - Title: the normalized fully-qualified test name (the `[Flaky Test] ` prefix is added automatically), e.g.
...
- `T` is **not** in the Step 1b `flakyTests` at all (zero observed failures in the complete scan), **and**
...
If neither new-flake (Step 5) nor backlog (Step 5b) candidates qualify, open no PR. Emit a `noop`
only if no issue or comment action is being declared either.
...
## Step 7 — Validate affected projects

Establish the SDK, runner, and target frameworks from the checked-out `global.json`,
`src/Directory.Build.props`, and `src/Directory.Build.targets`. The existing
[test/build guidance](../../../skills/running-unit-tests/SKILL.md) is a reference, not evidence that
its commands match every SDK/runner version; confirm version-specific help before selecting flags.
Compile each affected test project with the pinned SDK and restored prerequisites; escalate to the
repository build only for an uncovered integration need.
Do not install an arbitrary SDK, disable test isolation, or run repeated flake-reproduction loops.

A compile error in an edited file (malformed attribute, invalid `TestPlatforms`, dangling syntax)
means the edit must be corrected or dropped. A restore/feed/SDK-access failure is not evidence that
the edit compiles: do not blindly retry it or misclassify a C# compiler error as environmental.
Mechanical attribute-only PRs may still be proposed when local compilation is environmentally
blocked, but explicitly state that limitation and require the relevant CI results before merge.
Do not assume CI was triggered merely because a PR safe output was accepted.

## Step 8 — Validate the diff and open ONE combined PR

First confirm the diff is **limited and correct**:

1. `git diff --name-only <recorded-base-sha>` — every changed path **must** be a `.cs` file under `src/`, and specifically a
...
- Give the PR a supported `temporary_id`, such as `#aw_quarantine`, and use that reference in the
  planned issue comments. Do not poll for the real PR number: creation happens after the agent exits.
- Emit at most one comment per tracking issue, combining new evidence with its quarantine action.
  Reserve the twelve-comment budget before emitting any comments; prioritize quarantine actions
  over un-quarantine confirmations and evidence-only updates. Shared issue URLs consume one comment,
  not one per test. Leave overflow in the run summary.
